...
function Sidebar ({ routes }: { routes: Routes[] }): ReactElement {
  ...
  return (
    <>
      ...
      <aside className={`hidden relative shadow-purple-100 shadow-md bg-white md:flex md:flex-col h-screen transition-all ease-in-out duration-500 ${isSidebarExpanded ? 'w-48' : 'w-16'} ${classes.sidebar}`} aria-label="Sidebar">
        <OutlinedButton aria-label='Toggle sidebar' onClick={() => toggleSidebar(prevVal => !prevVal)} className='!min-h-0 mx-0 absolute -right-7 !rounded-full flex justify-center items-center'>
